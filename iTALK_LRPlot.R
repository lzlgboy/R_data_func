...
###first, set the parameter
    ...
    link.arr.lwd[which(link.arr.lwd==Inf)] = max(link.arr.lwd[which(link.arr.lwd<Inf)])
    link.arr.lwd[which(link.arr.lwd==-Inf)] = min(link.arr.lwd[which(link.arr.lwd>-Inf)])
    link.arr.lty = NULL 
    link.arr.col = NULL
    link.arr.width = res$cell_to_logFC 
    link.arr.width[which(link.arr.width==Inf)] = max(link.arr.width[which(link.arr.width<Inf)])
    link.arr.width[which(link.arr.width==-Inf)] = min(link.arr.width[which(link.arr.width>-Inf)])
